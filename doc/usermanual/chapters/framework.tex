...
\apsq allows to load externally generated field maps for various quantities such as the electric field or the doping profile of the sensor.
These maps have to be provided as regularly-spaced meshes in one of the supported field file formats.
A conversion and interpolation tool to translate adaptive-mesh fields from TCAD applications to the format required by \apsq is provided together with the framework and is described in Section~\ref{sec:tcad_electric_field_converter}.
...
\begin{figure}[t]
 ...
 \begin{subfigure}[t]{0.12\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_example_q1}%
         \caption{Quarter pixel}
         \label{fig:maps:q1}
 \end{subfigure}
 \begin{subfigure}[t]{0.24\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_example_ht}%
         \caption{Half pixel}
         \label{fig:maps:ht}
 \end{subfigure}
 \begin{subfigure}[t]{0.24\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_example_f}%
         \caption{Full pixel}
         \label{fig:maps:f}
 \end{subfigure}
 \begin{subfigure}[t]{0.24\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_example_fi}%
         \caption{Four quarter pixels}
         \label{fig:maps:fi}
 \end{subfigure}%
  \caption{Examples for pixel geometries in field maps. The dark spot represents the pixel center, the red extend the electric field. Pixel boundaries are indicated with a dotted line where applicable.}
  \label{fig:maps}
\end{figure}
...
Depending on the symmetries of the pixel unit cell and the pixel grid, different geometries are supported as indicated in Figure~\ref{fig:maps}.
The field for a quarter of the pixel plane (Figure~\subref{fig:maps:q1}), for half planes (Figure~\subref{fig:maps:ht}) as well as for full planes (Figure~\subref{fig:maps:f}).
The size of the field is not limited to a single pixel cell, however, for some quantities such as the electric field only the volume within the pixel cell is used and periodic boundary conditions are assumed and expected.
Larger fields are for example useful for the weighting potential, where also potential differences to neighboring pixels are of interest.

A special case is the field presented in Figure~\subref{fig:maps:fi}. Here, the field is not centered at the pixel unit cell center, but at the corner of four adjacent rectangular pixels.
...
\begin{figure}[t]
 ...
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_hl}%
         \caption{Left Half}
         \label{fig:maps:halves:left}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_hr}%
         \caption{Right Half}
         \label{fig:maps:halves:right}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_ht}%
         \caption{Top Half}
         \label{fig:maps:halves:top}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_hb}%
         \caption{Bottom Half}
         \label{fig:maps:halves:bottom}
 \end{subfigure}%
  \caption{Location and orientation of the field map with respect to the pixel center when providing a half of the pixel plane. Here, $(0,0)$ denotes the pixel center, the red field portion is read from the field map and the green ones are replicated through mirroring.}
  \label{fig:maps:halves}
\end{figure}
...
\begin{figure}[t]
 ...
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_q1}%
         \caption{Quadrant I}
         \label{fig:maps:quadrants:i}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_q2}%
         \caption{Quadrant II}
         \label{fig:maps:quadrants:ii}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_q3}%
         \caption{Quadrant III}
         \label{fig:maps:quadrants:iii}
 \end{subfigure}%
 \begin{subfigure}[t]{0.25\textwidth}
         \centering
         \includegraphics[width=\textwidth]{fieldmapping_q4}%
         \caption{Quadrant IV}
         \label{fig:maps:quadrants:iv}
 \end{subfigure}%
  \caption{Location and orientation of the field map with respect to the pixel center when providing one quadrant in the pixel plane. Here, $(0,0)$ denotes the pixel center, the red field portion is read from the field map and the green ones are replicated through mirroring.}
  \label{fig:maps:quadrants}
\end{figure}


The parameter \parameter{field_mapping} of the respective module defines how the map read from the mesh file should be interpreted and applied to the cell, and the following possibilities are available:
\begin{itemize}
\item \parameter{FULL} - the map is interpreted as field spanning the full Euclidean angle and aligned on the center of the pixel unit cell. No transformation is performed, but field values are obtained from the map with respect to the pixel center.
\item \parameter{FULL_INVERSE} - the map is interpreted as full field, but with inverse alignment on the pixel corners as shown in Figure~\subref{fig:maps:fi}. Consequently, the field value lookup from the four quadrants take into account their offset.
\item \parameter{HALF_LEFT} - the map represents the left Euclidean half-plane, aligned at the $y$ axis through the center of the pixel unit cell. Field values in the other half-plane are obtained by mirroring at the $y$ axis as indicated in Figure~\subref{fig:maps:halves:left}.
\item \parameter{HALF_RIGHT} - the map represents the right Euclidean half-plane, aligned at the $y$ axis through the center of the pixel unit cell. Field values in the other half-plane are obtained by mirroring at the $y$ axis as indicated in Figure~\subref{fig:maps:halves:right}.
\item \parameter{HALF_TOP} - the map represents the top Euclidean half-plane, aligned at the $x$ axis through the center of the pixel unit cell. Field values in the other half-plane are obtained by mirroring at the $x$ axis as indicated in Figure~\subref{fig:maps:halves:top}.
\item \parameter{HALF_BOTTOM} - the map represents the bottom Euclidean half-plane, aligned at the $x$ axis through the center of the pixel unit cell. Field values in the other half-plane are obtained by mirroring at the $x$ axis as indicated in Figure~\subref{fig:maps:halves:bottom}.
\item \parameter{QUADRANT_I} - the map represents the quadrant of the plane where both vector components have a positive sign. Field values in the other three quadrants are obtained by mirroring at the axes of the coordinate system as show in Figure~\subref{fig:maps:quadrants:i}.
\item \parameter{QUADRANT_II} - the map represents the quadrant of the plane where the vector component $x$ has a negative and $y$ a positive sign. Field values in the other three quadrants are obtained by mirroring at the axes of the coordinate system as show in Figure~\subref{fig:maps:quadrants:ii}.
\item \parameter{QUADRANT_III} - the map represents the quadrant of the plane where both vector components have a negative sign. Field values in the other three quadrants are obtained by mirroring at the axes of the coordinate system as show in Figure~\subref{fig:maps:quadrants:iii}.
\item \parameter{QUADRANT_IV} - the map represents the quadrant of the plane where the vector component $x$ has a positive and $y$ a negative sign. Field values in the other three quadrants are obtained by mirroring at the axes of the coordinate system as show in Figure~\subref{fig:maps:quadrants:iv}.
\end{itemize}

All axes mentioned here are Cartesian axes aligning with the local coordinate system of the sensor, described in Section~\ref{sec:coordinate_systems}, and passing through the center of the pixel unit cell regarded.
It should be noted that some of these mappings are equivalent to rotating or mirroring the field before loading it in \apsq, and are only provided for convenience.
