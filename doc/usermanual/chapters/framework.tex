...
\item \parameter{implant_offset}: 2D values in the x-y plane, defining the offset of the implant from the center of the pixel cell.
The implant cannot overlap with neighboring pixel cells and the maximum values for the offset is therefore restricted to half the pixel pitch minus half the implant size in the respective coordinate.
