...
Different geometries of field maps can be provided, depending on the symmetries within the pixel cell.
The parameter \parameter{field_mapping} defines how the map read from the mesh file should be applied to the cell, and the following possibilities are available:
\begin{itemize}
\item \parameter{FULL}
\item \parameter{FULL_INVERSE}
\item \parameter{HALF_RIGHT}
\item \parameter{HALF_LEFT}
\item \parameter{HALF_BOTTOM}
\item \parameter{HALF_TOP}
\item \parameter{QUADRANT_I}
\item \parameter{QUADRANT_II}
\item \parameter{QUADRANT_III}
\item \parameter{QUADRANT_IV}
\end{itemize}
